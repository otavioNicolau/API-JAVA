...
class SecurityValidationTest {
  ...
  void shouldRejectFileExceedingMaxSize() throws Exception {
    ...
    MockMultipartFile largeFile = new MockMultipartFile(
        "files", "large-file.pdf", "application/pdf", largeFileContent);
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.message").value(
            "File 'large-file.pdf' exceeds maximum size of 50 MB. File size: 60.00 MB"));
  }

  @Test
  void shouldRejectTooManyFiles() throws Exception {
    // Given - 11 arquivos (excede limite de 10)
    MockMultipartFile file1 = new MockMultipartFile("files", "file1.pdf", "application/pdf", "content1".getBytes());
    MockMultipartFile file2 = new MockMultipartFile("files", "file2.pdf", "application/pdf", "content2".getBytes());
    MockMultipartFile file3 = new MockMultipartFile("files", "file3.pdf", "application/pdf", "content3".getBytes());
    MockMultipartFile file4 = new MockMultipartFile("files", "file4.pdf", "application/pdf", "content4".getBytes());
    MockMultipartFile file5 = new MockMultipartFile("files", "file5.pdf", "application/pdf", "content5".getBytes());
    MockMultipartFile file6 = new MockMultipartFile("files", "file6.pdf", "application/pdf", "content6".getBytes());
    MockMultipartFile file7 = new MockMultipartFile("files", "file7.pdf", "application/pdf", "content7".getBytes());
    MockMultipartFile file8 = new MockMultipartFile("files", "file8.pdf", "application/pdf", "content8".getBytes());
    MockMultipartFile file9 = new MockMultipartFile("files", "file9.pdf", "application/pdf", "content9".getBytes());
    MockMultipartFile file10 = new MockMultipartFile("files", "file10.pdf", "application/pdf", "content10".getBytes());
    MockMultipartFile file11 = new MockMultipartFile("files", "file11.pdf", "application/pdf", "content11".getBytes());
    ...
    mockMvc
        .perform(
            multipart("/api/v1/jobs")
                ...
                .file(file6).file(file7).file(file8).file(file9).file(file10).file(file11)
                .header("X-API-Key", "test-key-67890")
                .param("operation", "MERGE"))
        .andExpect(status().isPayloadTooLarge())
        .andExpect(jsonPath("$.code").value("MAX_FILES_EXCEEDED"))
        .andExpect(jsonPath("$.message").value(
            "Maximum 10 files allowed per job, but 11 files provided"));
  }
  ...
  void shouldRejectTooManyInputFiles() throws Exception {
    ...
    List<String> inputFiles = List.of(
        "job-1/file1.pdf", "job-1/file2.pdf", "job-1/file3.pdf", "job-1/file4.pdf",
        "job-1/file5.pdf", "job-1/file6.pdf", "job-1/file7.pdf", "job-1/file8.pdf",
        "job-1/file9.pdf", "job-1/file10.pdf", "job-1/file11.pdf");
