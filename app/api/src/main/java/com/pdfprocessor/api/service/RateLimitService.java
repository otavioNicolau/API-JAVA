...
/**
 * Service for managing API rate limiting per API key. Implements a sliding window rate limiter with
    ...
    synchronized (window) {
      ...
      if (window.requestCount.get() >= MAX_REQUESTS_PER_HOUR) {
        long minutesUntilReset =
            WINDOW_SIZE_MINUTES - ChronoUnit.MINUTES.between(window.windowStart, now);
